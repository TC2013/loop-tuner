...
export function averageNetTempBasalsByPeriod(netTempBasals, options) {
  console.log('netTempBasals After creation before filling in default profile entries', netTempBasals)
  ...
  let averageNetTempBasals = new Array(days * 288).fill(null);  // Array to hold the average basal rate for each 5-minute period
  console.log('averageNetTempBasals was created', averageNetTempBasals)
  let dayCounter = 0 
  ...
  netTempBasals.forEach((obj, index) => {
    ...
    let currentPosition = (288 * dayCounter) + tbEnd.getHours() * 12 + Math.floor(tbEnd.getMinutes() / 5)
 ...
 (function (arr) {
    ...
    console.log('sumNetTempBasals: I SHOULD EQUAL THE VALUE BELOW', sumNetTempBasals)
    console.log('This amount should match the amount above:', sum)  
    return sum;
  })(averageNetTempBasals);
  console.log('averageNetTempBasals', averageNetTempBasals)
  console.log('averageNetTempBasals00000000000',averageNetTempBasals)
  ...
  let prevTimestamp;
  netTempBasals.forEach((obj) => {
    let i = new Date(obj.created_at).getHours() * 12 + Math.floor(new Date(obj.created_at).getMinutes() / 5);
    // console.log('index',i, 'obj.created_at date',new Date(obj.created_at).getDate())
    // if(prevTimestamp){console.log('This is the prevTimestamp date',new Date(prevTimestamp).getDate())}

    if (!prevTimestamp || (new Date(obj.created_at).getDate() !== new Date(prevTimestamp).getDate()) || (i !== prevI) ){
      counters[i]++;
      // console.log('counters[i]++',counters[i],'for i =',i)
    }
    prevTimestamp = obj.created_at;
    prevI = i
  });
  ...
  {
    if (counters[i] > 0) {
      averageNetTempBasals[i] = averageNetTempBasals[i] / counters[i];
    }
  }
  console.log('averageNetTempBasalsByPeriod',averageNetTempBasals)
  console.log('sum of averageNetTempBasalsByPeriod',averageNetTempBasals.reduce((a, b) => a + b, 0))
...
export function getNetTempBasals(){
  ...
  for(let i = 0; i < tempBasals.length-1; i++){
    ...
    if(i == 0 && tbStart.getHours() != 0 && tbStart.getMinutes() != 0) {
      prevTbStart = new Date(tbStart).toLocaleDateString().split('T')[0] + "T" + "00:00"
      prevTbEnd = new Date(tempBasals[i].created_at)
      pushBasalProfiles(basalProfiles, prevTbStart, netTempBasals, tbStart) 
    } 
...
console.log('netTempBasals just created',netTempBasals)
...
function pushBasalProfiles(basalProfiles: Array<BasalProfile>, tbEnd: Date, netTempBasals: Array<TempBasal>, nextTbStart: Date){
  ...
  // console.log('This is the profile after the first part of pushBasalProfiles runs:',profile)
  // I believe the problem is with this code below. When midnight is hit and it had to create a profile, it doesn't loop back to the begging of the profile to start the next day.!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  for(let i = 0; i < profile.length -1; i++){
      ...
      if (tbEnd >= bStart && tbEnd <= bEnd){
          ...
          if(newBEnd = bFinal){newBEnd = new Date(nextTbStart)}// TO DO: Fix me: This enters the current basal rate until the nextTbStart when at the end of the day.
    ...
    //TODO: Write a loop to find the right basal rate for the start of the next day. Also, if their is a profile change mid-day.
      // if(bEnd = final && tbEnd < nextTbStart){
      //   let newBEnd = new Date(nextTbStart)
      //   let created_at = new Date(tbEnd)
      //   let duration = (newBEnd.getTime() - created_at.getTime()) / (1000 * 60)
      //   let rate = profile[0].value
      //   netTempBasals.push({
      //     rate: rate,
      //     duration: duration,
      //     created_at: created_at,
...
export async function avgBolusBasalByPeriod(options) {
  ...
  let mealBolusTotalsAfterProcessing = sumMealBolusInsulinByDate(allBolusArray)
